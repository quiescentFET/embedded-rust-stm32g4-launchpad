...
3. Needs confirmation, but assuming using HSI, the internal 16MHz oscillator. ~~Should use LSE, the more stable external 32.768KHz crystal (X2).~~ Turns out the LSE is for RTC and non sysclk use. For accuracy, use HSE, the external 24MHz crystal (X1). Perhaps switch time driver to LSE?
